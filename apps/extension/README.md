...
- **Custom Hotkeys**: Assign keyboard shortcuts to text search engines
- **Platform Aware**: Mac/Windows/Linux optimized shortcut display
- **Smart Detection**: Only works on text selections, safe in input fields
- **Real-time Recording**: Press keys to assign shortcuts instantly
- **Modifier Combinations**: Support for Ctrl/Cmd + Alt/Option + Shift + Key
...
4. **Setup Shortcuts**: Assign keyboard shortcuts to text search engines
5. **Start Searching**:
   - **Right-click**: Select text/image → Right-click → Choose engine
   - **Keyboard**: Select text → Press assigned shortcut → Instant search
...
- **Keyboard Shortcut**: Assign hotkeys (text search only)
...
│   │   │   ├── KeyboardShortcutRecorder.tsx # Shortcut recording component
...
│   │   ├── content/       # Keyboard shortcut handler
│   │   │   └── index.ts   # KeyboardShortcutManager class
...
**Output:** `right-click-search-v0.1.0-20250116.zip`
...
1. Select text on any webpage
2. Press assigned keyboard shortcut (e.g., Cmd+Shift+Y for YouTube)
3. Instant search results open in new tab
4. Works anywhere except input fields
